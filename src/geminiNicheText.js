...
function nicheRules(niche) {
  if (niche === 'buyer_education') {
    return {
      ...
      purpose: 'buyer-protection education about the purchase process, booking, agreements, builder commitments, possession, common areas, parking allocation, association/maintenance obligations and other transaction risks',
      visualWorld: 'real buyer meetings, booking forms, sale agreements, handover documents, parking allocations, common areas, maintenance notices, possession checklists and realistic Coimbatore/Tamil Nadu neighbourhoods',
      cautions: 'Do not turn this into construction engineering. Waterproofing, electrical systems, earthing, plumbing, RCC, reinforcement, masonry, structural defects, workmanship, materials and building-system quality checks belong to OliveTree Builders even when framed as a buyer inspection or handover checklist. Do not invent project approvals, builder claims, maintenance charges, legal status, dimensions, prices, loan eligibility or amenities.'
    };
  }
  if (niche === 'documentation' || niche === 'land_selection') {
    return {
      brand: 'OliveTree SafeBuy',
      purpose: niche === 'documentation'
        ? 'property documentation, approvals, title, ownership, registration and legal-risk education'
        : 'land-selection education about access, boundaries, shape, frontage, drainage/flood exposure, approach road and site suitability',
      visualWorld: 'real document desks, sale deeds, EC-style paperwork, approval drawings, survey sketches, boundary stones, measuring tape, site inspection, access roads, plotted land, drainage, EB poles and realistic Coimbatore/Tamil Nadu surroundings',
      cautions: 'Construction systems must not be converted into SafeBuy content. Waterproofing, electrical/earthing, plumbing, RCC, reinforcement, masonry, structural defects, workmanship, materials and site-execution quality belong to OliveTree Builders. Do not state a legal conclusion unless explicitly supported by the source. Do not invent approval status, survey numbers, patta status, EC results, road ownership, setbacks, FSI, guideline value, litigation status or government records.'
    };
  }
  if (niche === 'construction') {
    return {
      brand: 'OliveTree Builders',
      purpose: 'practical residential construction education covering structural systems, MEP systems, materials, workmanship, waterproofing, electrical/earthing, plumbing and site quality control',
      visualWorld: 'real active construction sites, footing excavation, reinforcement steel, shuttering, concrete pours, brick/block masonry, curing, plumbing and electrical conduits, earthing, distribution boards, waterproofing, roof slab, plastering, tile work, material inspection, workers using normal tools and realistic Coimbatore/Tamil Nadu site conditions',
  ...
  for (const model of models.slice(0, 6)) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      ...
      if (res.ok) {
        ...
        if (!Array.isArray(pack.hashtags) || pack.hashtags.length < 2 || pack.hashtags.length > 3) throw new Error('Gemini must return 2 or 3 hashtags');
